...
void rhoattack()
{
	...
	for(int i=0;i<1000000;i++)
	{
		...
		result1 = iter(pad1);
		list[i].input = str1;
		list[i].output = result1;
		for (int j = 0; j < i; j++)
		{
			if (result1.substr(0, 32) == list[j].output.substr(0, 32))
			{
				cout <<"输入字符串为："<< list[j].input << "\t" << list[i].input << endl;
				cout << result1 << endl;
				cout << list[j].output << endl;
				return;
			}
		}
		num = (2 * num + 1) % 1000000;
